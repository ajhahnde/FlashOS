...
use flash_runtime::eval::{ControlKind, RuntimeError, RuntimeErrorKind, evaluate};
use flash_runtime::operation::OperationError;
...
fn literals_and_operators_produce_the_last_expression_value() {
    ...
    assert_eq!(
        ok("1.0 / 4.0"),
        Value::from(flash_runtime::FiniteFloat::new(0.25).unwrap())
    );
