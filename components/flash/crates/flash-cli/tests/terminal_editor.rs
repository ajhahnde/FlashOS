...
use flash_cli::terminal_editor::key::{Key, KeyDecoder};
...
use flash_cli::terminal_editor::buffer::EditBuffer;
...
use flash_cli::terminal_editor::history::HistoryRing;
use flash_cli::terminal_editor::render::render_line;
...
fn a_prompt_wider_than_the_terminal_renders_no_text() {
    // A three-column prompt fits four columns but leaves only one cell. The
    // cursor sits past the last character and claims that cell, so no text
    // is visible. Pinning the behaviour is worth more than pretending it
    // degrades gracefully.
...
fn editing_mid_line_keeps_text_visible_to_the_right_of_the_cursor() {
    // Cursor at character 15 of 20 in a twelve-cell window. Anchoring the
    // window on the cursor alone would end it there and show "efghijklmnop"
    // at column 15, hiding every character still to come. The quarter-row
    // margin shifts the window right, so more text stays on screen.
    let text = "abcdefghijklmnopqrst";
...
fn an_unreadable_terminal_size_falls_back_to_eighty_columns() {
    ...
    // three-column prompt leaves a 77-cell window, whose last cell is reserved
    // for the cursor, so 76 of the 100 characters are visible and the cursor
    // lands on column 80.
