...
// lib/syscall_defs.zig hard-coded (SYS_WRITE=33 with x0=fd[1=stdout],
// x1=buf, x2=len; SYS_EXIT=2). The payload string lives in the same
// .text section, reached PC-relative via `adr` so the loader is free to
// map this segment anywhere — the blob is fully position-independent at
// instruction granularity even though the ELF itself is ET_EXEC.
