...
usage()
{
    ...
    echo "    -c CONFIG: The name of the config, e.g. flashos."
    echo "               Determines the name of the image, build/ARCH/CONFIG/harddrive.img"
    echo "               e.g. build/x86_64/flashos/harddrive.img"
    echo "               Determines the name of FILESYSTEM_CONFIG if none is specified."
    echo "               Defaults to the basename of FILESYSTEM_CONFIG, or 'flashos'"
    ...
    echo "    Examples:  ./build.sh -c flashos live - make build/x86_64/flashos/redox-live.iso"
    echo "               ./build.sh qemu - make build/x86_64/flashos/harddrive.img and"
    echo "                                 run it in qemu"
