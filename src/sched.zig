...
// is the whole point.
inline fn free_page_list(list: anytype, comptime field: ?[]const u8) void {
    var j: usize = 0;
    while (j < list.len) : (j += 1) {
        const v: u64 = if (field) |f| @field(list[j], f) else list[j];
        if (v != 0) free_page(v);
    }
}
...
export fn release_user_mm(t: *TaskStruct) void {
            ...
            if (task[i]) |c| {
                if (c.parent == current.?) {
                    ...
                    if (c.state == TASK_ZOMBIE) {
                        ...
                        task[i] = null;
                        if (c.kstack != 0) free_kernel_page(c.kstack);
