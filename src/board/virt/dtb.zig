...
pub const Dtb = struct {
    ...
    fn propName(self: *const Dtb, name_off: u32) []const u8 {
        // Bound the NUL scan to the blob: a corrupt name_off must not
        // walk off the end of the strings region.
        const abs = self.off_strings + name_off;
        if (abs >= self.total_size) return "";
        const start = self.base + abs;
        const max = self.total_size - abs;
        var len: usize = 0;
        while (len < max and start[len] != 0) : (len += 1) {}
        return start[0..len];
    }
        ...
        while (off + 4 <= end) {
            ...
            switch (tok) {
                FDT_BEGIN_NODE => {
                    ...
                    var name_len: u32 = 0;
                    while (off + name_len < end and self.base[off + name_len] != 0) : (name_len += 1) {}
                ...
                FDT_PROP => {
                    ...
                    off += 8;
                    // A corrupt plen must not slice past the struct block.
                    if (plen > end - off) return null;
...
// Little FDT writer for the tests below — appends big-endian tokens and
// raw prop bytes into a fixed buffer. Not referenced by the kernel build.
const TestBlob = struct {
    buf: [256]u8 = undefined,
    w: usize = 0,
    fn u32be(self: *TestBlob, v: u32) void {
        std.mem.writeInt(u32, self.buf[self.w..][0..4], v, .big);
        self.w += 4;
    }
    fn raw(self: *TestBlob, b: []const u8) void {
        @memcpy(self.buf[self.w..][0..b.len], b);
        self.w += b.len;
    }
};

test "beU32/beU64 decode big-endian" {
    const b4 = [_]u8{ 0x12, 0x34, 0x56, 0x78 };
    try testing.expectEqual(@as(u32, 0x12345678), beU32(&b4));
    const b8 = [_]u8{ 0x01, 0x23, 0x45, 0x67, 0x89, 0xAB, 0xCD, 0xEF };
    try testing.expectEqual(@as(u64, 0x0123456789ABCDEF), beU64(&b8));
}

test "alignUp4 rounds up to a 4-byte boundary" {
    try testing.expectEqual(@as(u32, 0), alignUp4(0));
    try testing.expectEqual(@as(u32, 4), alignUp4(1));
    try testing.expectEqual(@as(u32, 4), alignUp4(4));
    try testing.expectEqual(@as(u32, 8), alignUp4(5));
}

test "dtb parses reg base + interrupt from a hand-built FDT" {
    var t = TestBlob{};
    // --- structure block @ off 0 ---
    t.u32be(FDT_BEGIN_NODE);
    t.raw(&[_]u8{ 0, 0, 0, 0 }); // empty node name, NUL + 4-byte pad
    // compatible = "testdev"
    t.u32be(FDT_PROP);
    t.u32be(8); // plen
    t.u32be(0); // nameoff → "compatible"
    t.raw("testdev\x00");
    // reg = addr 0x09000000, size 0x1000 (2 addr + 2 size cells = 16 B)
    t.u32be(FDT_PROP);
    t.u32be(16);
    t.u32be(11); // nameoff → "reg"
    t.u32be(0);
    t.u32be(0x09000000); // addr (be64 split hi/lo)
    t.u32be(0);
    t.u32be(0x1000); // size
    // interrupts = SPI (type 0), irq 7, flags 4
    t.u32be(FDT_PROP);
    t.u32be(12);
    t.u32be(15); // nameoff → "interrupts"
    t.u32be(0);
    t.u32be(7);
    t.u32be(4);
    t.u32be(FDT_END_NODE);
    t.u32be(FDT_END);
    const size_struct: u32 = @intCast(t.w);
    const off_strings: u32 = @intCast(t.w);
    // --- strings block ---
    t.raw("compatible\x00"); // name_off 0
    t.raw("reg\x00"); // name_off 11
    t.raw("interrupts\x00"); // name_off 15
    const total: u32 = @intCast(t.w);
    ...
    try testing.expectEqual(@as(?u64, 0x09000000), d.findDeviceBase("testdev"));
    try testing.expectEqual(@as(?u64, 0x09000000), d.findRegN("testdev", 0));
    try testing.expectEqual(@as(?u64, null), d.findRegN("testdev", 1)); // only one reg entry
    try testing.expectEqual(@as(?u32, 39), d.findInterrupt("testdev")); // SPI 7 → 7 + 32
    try testing.expectEqual(@as(?u64, null), d.findDeviceBase("nonexist"));
}

test "dtb rejects a corrupt prop length without reading past the blob" {
    var t = TestBlob{};
    t.u32be(FDT_BEGIN_NODE);
    t.raw(&[_]u8{ 0, 0, 0, 0 });
    t.u32be(FDT_PROP);
    t.u32be(0xFFFFFFFF); // corrupt plen — claims a value larger than the blob
    t.u32be(0); // nameoff → "compatible"
    t.u32be(FDT_END_NODE);
    t.u32be(FDT_END);
    const size_struct: u32 = @intCast(t.w);
    const off_strings: u32 = @intCast(t.w);
    t.raw("compatible\x00");
    const total: u32 = @intCast(t.w);
    ...
    // Must fail closed (null), not slice/scan past the struct block.
    try testing.expectEqual(@as(?u64, null), d.findDeviceBase("anything"));
}
