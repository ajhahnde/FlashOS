// BCM2711 EMMC2 SDHCI driver — PIO block I/O (v0.4.0).
//
...
// stack. v0.4.0: init() + write_block(LBA 2064) + read_block +
...
inline fn emmc_write(reg: *volatile u32, val: u32) void {
    reg.* = val;
    if (low_clock) delay_us(11); // 4 SD-clk @ 390 kHz ≈ 10.3 µs, round up
}
...
pub fn init() i32 {
    ...
    //     from this 3.3 V default (v0.4.0); 1.8 V UHS-I
    ...
    // Stage 3d's FAT32 backend reads + writes through block_dev.sd_dev;
...
pub fn read_block(lba: u32, buf: *[512]u8) callconv(.c) i32 {
    ...
    r.interrupt = INTERRUPT_DATA_DONE;
    return 0;
}
