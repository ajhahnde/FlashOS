...
test "get_free_page returns the 0 sentinel when the pool is exhausted" {
    ...
    for (0..MALLOC_PAGES) |i| mem_map[i] = 1;
...
test "mem_map_reserve_above caps the pool at the RAM end" {
    ...
    // Drain to the cap; the page just below ram_end is the last legal one.
    for (0..in_ram_pages - 1) |_| _ = get_free_page();
