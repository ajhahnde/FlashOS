...
fn readdir(_: *vfs.SuperBlock, path_ptr: [*]const u8, path_len: usize, index: u64, out: *vfs.Dirent) callconv(.c) c_int {
    ...
    const prefix: []const u8 = blk: {
        if (path.len == 1 and path[0] == '/') break :blk "/";
    ...
    var emitted: u64 = 0;
    var last_child: []const u8 = &.{};
    var have_last = false;
    while (it.next() catch return -1) |e| {
        const de = initramfs.directEntry(e.name, prefix) orelse continue;
        if (have_last and bytesEql(de.child, last_child)) continue; // adjacent dup
        last_child = de.child;
        have_last = true;
        if (emitted == index) {
            out.* = .{};
...
test "initramfs_backend: readdir synthesises dirs with adjacent-dup collapse" {
    ...
    const fixture = comptime initramfs.buildFixture(&.{
