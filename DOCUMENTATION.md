...
- **PID-1 → login → fsh hand-off.** PID 1 stays the test harness:
  ...
  per-session `[Debug] login OK` marker, then **forks** a child that drops
  ...
  **Reaching the interactive prompt is the boot success signal:** fsh
  prints `[Debug] fsh init OK` at REPL entry and shows `#` (root) or `$`
  ...
  **third** `fsh init OK` (then SIGTERMs) and asserts exactly 3 × both
...
- `login` — the login-lifecycle capstone. Injects a two-session
  ...
  session emits the `[Debug] login OK` / `[Debug] fsh init OK` markers, so
...
`[Debug] login OK` and `[Debug] fsh init OK` markers each appear **three
...
(the `[Debug] fsh init OK` marker the watchdog matches, see §4 PID-1 → fsh
...
| `user_space/fsh/tokenize.zig`       |   11 | (PID-1 hand-off)                                                                                  | pure whitespace split + single-pipe decomposition; the shell driver (`fsh.zig`) is integration-only via the PID-1 → fsh hand-off (the `[Debug] fsh init OK` boot success marker)                                                                                                                     |
...
| `[Debug] login OK`          | Auth success marker — `/bin/login` verified the credentials and dropped privilege; the QEMU watchdog asserts it appears exactly once |
| `[Debug] fsh init OK`          | Boot success marker — fsh has reached its interactive REPL; the QEMU watchdog and the real-HW `picapture` helper both wait on it |
...
31 per-scenario checkpoints + 1 boot baseline, and the `[Debug] login OK`
and `[Debug] fsh init OK` markers emitted.
