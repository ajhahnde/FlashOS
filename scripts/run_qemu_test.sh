...
# login lifecycle, `[Debug] fsh init OK` (user_space/fsh/fsh.zig) appears
...
#   * 0  on the 3rd `[Debug] fsh init OK` with no `[FAIL]` / `ERROR CAUGHT`
...
# no typist and reaches the 3rd `[Debug] fsh init OK`. Without it the boot
...
#         self-test announce
#   *  3 × `[Debug] login OK` + 3 × `[Debug] fsh init OK` (two scripted
...
while kill -0 "$QEMU_PID" 2>/dev/null; do
    ...
    if [ "$(grep -cF "[Debug] fsh init OK" "$LOG" || true)" -ge 3 ]; then
        status=ready
        break
    fi
...
if [ "$rpi_chk" -gt 0 ]; then
    ...
    ok_chk=$virt_chk
    ok_base=$(grep -cF "free_pages: 000000000003be58" "$LOG" || true)
    chk_label="0x3be4a"; base_label="0x3be58"
else
    echo "FAIL (no known checkpoint pattern): neither 0xbbff2 (rpi4b) nor 0x3be4a (virt) found" >&2
    tail -n 50 "$LOG" >&2
    exit 1
fi
...
login_ok=$(grep -cF "[Debug] login OK" "$LOG" || true)
fsh_ok=$(grep -cF "[Debug] fsh init OK" "$LOG" || true)
