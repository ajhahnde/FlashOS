...
# scripts/check_whitespace_hygiene.sh; scope: the retained assembly trees
# (arch/, board/) and crate-owned asm under crates/. armstub/ stays out of
# scope: it is adapted upstream code and keeps the upstream hex style.
#
...
# Search the asm trees for hex literals with lowercase a-f (0x[0-9]*[a-f][0-9a-f]*).
...
hits="$(grep -rnE "0x[0-9a-f]*[a-f][0-9a-f]*" arch/ board/ crates/ \
...
if [ -n "$hits" ]; then
    echo "check_hex_hygiene: lowercase hex literals found in the asm trees:" >&2
    printf '%s\n' "$hits" >&2
